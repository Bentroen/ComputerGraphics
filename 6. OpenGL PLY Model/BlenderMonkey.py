...
class BlenderMonkey:
    def __init__(self):
        ...
        vertices = array("f")
        indices = array("I")
        with open("objs/blender_monkey.ply", "r") as f:
            for line in f:
                parts = line.split()
                if state == 0:  # HEADER
        ...
        GL.glVertexAttribPointer(
            1, 1, GL.GL_FLOAT, GL.GL_FALSE, stride, intensityPointer
        )
    ...
    def draw(self):
        GL.glBindVertexArray(self.arrayBufferId)
        GL.glDrawElements(
            GL.GL_TRIANGLES, self.N, GL.GL_UNSIGNED_INT, ctypes.c_void_p(0)
        )
